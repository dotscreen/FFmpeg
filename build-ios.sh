...
function build_arch() {
    ...
    SDK_HOME=(${PLATFORM_HOME}/Developer/SDKs/${arch}*.sdk)
    TOOLS_PATH=${PLATFORM_HOME}/Developer/usr/bin/
    EXTRA_FLAGS="-arch ${arch_code} -isysroot ${SDK_HOME}"

    echo "SDK $arch: $SDK_HOME"
    make distclean > /dev/null 2>&1 
    ./configure --cross-prefix=${TOOLS_PATH} $FFMPEG_GENERAL_OPTIONS $arch_options --sysroot=${SDK_HOME} "--extra-cflags=${EXTRA_FLAGS}" "--extra-ldflags=${EXTRA_FLAGS}"
    make -j3 || exit 1
...
FFMPEG_GENERAL_OPTIONS="${FFMPEG_GENERAL_OPTIONS} --disable-debug --disable-vda --enable-small --disable-mmx"

build_arch iPhoneSimulator i386 "--arch=i386"
build_arch iPhoneOS armv7 "--cpu=cortex-a8 --arch=arm --enable-neon"
